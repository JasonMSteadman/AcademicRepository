...
T SinglyLinkedList<T>::getAtIndex(const unsigned int index) const
{
	...
	cout << "Sorry that is out of the bounds of this linked list." << endl;
	throw 1;
...
{
	if (!index) {  //If first
		insertFront(value);		//Thanks for helping out ;)
	}
	else if ((index + 2) == count) {		//If last
		insertBack(value);		//Thanks for helping out ;)
	}  
	else if (index <= count) {		//If more than two
		Node<T> * temp2 = front;
...
{	
	if (index < count) {	
		if ((!index)) {		//If first
			...
			Node<T> * curr = temp->link;
			temp->link = curr->link;
			if (curr->link == nullptr) {		//If last	
				back = temp;
			}
			delete curr;
		}
...
void SinglyLinkedList<T>::deleteAllInstances(const T& value)
{
	...
	if (front != nullptr && front != back) {		//If more than one node
		Node<T> * temp = front;
		Node<T> * curr = temp;
		while (front->data == value) {   //If first
			front = front->link;
			delete curr;
			curr = temp = front;
			if (front == nullptr) {
	...
	else if (front != nullptr && front == back && front->data == value)  {		//If one
		delete front;
		front = back = nullptr;
	}
	else {
