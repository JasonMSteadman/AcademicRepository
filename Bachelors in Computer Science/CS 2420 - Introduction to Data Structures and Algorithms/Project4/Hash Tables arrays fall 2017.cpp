...
arrayHashTable<KEY, VALUE>::arrayHashTable(int arraySize) {
	...
	keyArray = std::make_unique< KEY[] >(arraySize);
	valueArray = std::make_unique< VALUE[] >(arraySize);
	statusArray = std::make_unique< int[] >(arraySize);
...
arrayHashTable<KEY, VALUE>::arrayHashTable(arrayHashTable<KEY, VALUE>&& obj) {
	...
	keyArray = std::make_unique< KEY[] >(std::move(obj.keyArray));
	valueArray = std::make_unique< VALUE[] >(std::move(obj.valueArray));
	statusArray = std::make_unique< int[] >(std::move(obj.statusArray));
...
arrayHashTable<KEY, VALUE>& arrayHashTable<KEY, VALUE>::operator=(arrayHashTable<KEY, VALUE>&& obj) {
	...
	keyArray = std::move(obj.keyArray);
	valueArray = std::move(obj.valueArray);
	statusArray = std::move(obj.statusArray);
...
std::hash< KEY > myHash;
size_t myKey = myHash(key) % arraySize;		
if (statusArray[myKey] < 1) {		//Check container
	keyArray[myKey] = key;
	valueArray[myKey] = value;
	statusArray[myKey] = 1;
}
else {
	int count = arraySize;
	while (count) {		//Looping through array once
		++myKey;
		if (myKey == arraySize) {		//Wrap key
			myKey = 0;
		}
		if (statusArray[myKey] < 1) {		//Search for free container
			keyArray[myKey] = key;
			valueArray[myKey] = value;
			statusArray[myKey] = 1;
			break;
...
void arrayHashTable<KEY, VALUE>::add(const KEY& key, VALUE&& value) {
	...
	if (statusArray[myKey] < 1) {		//Check container
		keyArray[myKey] = key;
		valueArray[myKey] = std::move(value);
		statusArray[myKey] = 1;
	}
	else {
		int count = arraySize;
		while (count) {		//Looping through array once
			++myKey;
			if (myKey == arraySize) {		//Wrapping array to start
				myKey = 0;
			}
			if (statusArray[myKey] < 1) {		//Search for free container
				keyArray[myKey] = key;
...
bool arrayHashTable<KEY, VALUE>::exists(const KEY& key) const {
	...
	std::hash< KEY > myHash;
	size_t myKey = myHash(key) % arraySize;
	for (int i = 0; i < arraySize; ++i) {		//Looping through array once
		if (myKey == arraySize) {		//Wrapping key to start
			myKey = 0;
		}
		if (statusArray[myKey] == 0) {		//Checking for empty containers
			return false;
		}
		if (statusArray[myKey] == 1 && keyArray[myKey] == key) {		//Checking status and key
			return true;
		}
		else {		
			++myKey;
